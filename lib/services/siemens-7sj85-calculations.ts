 ...
 */
 ...
 static calculateTotalLoadOtherBurden(devices: any): number {
 if (Array.isArray(devices)) {
 return devices.reduce((sum, d) => sum + (typeof d === 'number' ? d : (d.burden_va || d.burden || 0)), 0);
 }
 if (typeof devices === 'object' && devices !== null) {
 return Object.values(devices).reduce((sum: number, val: any) => sum + (typeof val === 'number' ? val : (val?.burden_va || 0)), 0);
 }
 return 0;
 }
}
...
export class Siemens7SJ85Calculator {
 ...
 // ------------------------------------------------------------
 const ct_length = input.ct_wiring.ct_conductor_length_m ?? input.ct_wiring.conductor_length_m ?? 0;
 const ct_r20 = input.ct_wiring.ct_resistance_w_km_20c ?? input.ct_wiring.resistance_w_km_20c ?? 0;
 const cable_length_km = ct_length / 1000;
 const R_75C = ct_r20 * 1.21615;
 ...
 loop_resistance: loop_resistance_ohm,
 ...
 // ------------------------------------------------------------
 if (input.vt_wiring) {
 const vt_r20 = input.vt_wiring.vt_resistance_w_km_20c ?? input.vt_wiring.resistance_w_km_20c ?? 0;
 const vt_len = input.vt_wiring.vt_conductor_length_m ?? input.vt_wiring.conductor_length_m ?? 0;
 const VT_R_75C = vt_r20 * 1.21615;
 const vt_length_km = vt_len / 1000;
 ...
 // ------------------------------------------------------------
 // 3. FAULT CURRENT (STANDARD PAGES 2-4)
 // ------------------------------------------------------------
 ...
 tp_ms: tp * 1000,
 system_tp_ms: tp * 1000,
 through_fault_current_a: Itkmax * 0.8695,
 endzone1_fault_current_a: Itkmax * 0.8717,
 ...
 internal_burden_va: PE,
 wiring_burden_va: wiring_burden_va,
 devices_burden_va: PL_devices,
 total_burden_va: PL_total,
 rated_burden_PN_va: PN,
 device_count: Array.isArray(input.connected_devices) ? input.connected_devices.length : Object.keys(input.connected_devices || {}).length,
 devices: input.connected_devices,
 };

 // ------------------------------------------------------------
 // 5. CT ADEQUACY CHECK (Kssc METHOD, STANDARD PAGE 5-6)
 // ------------------------------------------------------------
 ...
 results.vk_required = required_kssc * input.ct_core.ct_resistance;
 results.vk_available = input.ct_core.vk_available ?? 400;
