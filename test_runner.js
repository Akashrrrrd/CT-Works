...
const { ExcelProcessor } = require('./lib/services/excel-processor');
...
async function run() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const validationResult = ExcelProcessor.processExcelBuffer(buffer);
      ...
      const ied = input.ieds.find(i => i.ied_name.includes('RED670') || i.ied_name.includes('7SJ85'));
      if (!ied) {
