...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!approvalId || !comments?.trim()) {
    return NextResponse.json({ error: 'approvalId and comments are required' }, { status: 400 });
  }
  ...
  await approvals.updateOne(
    ...
    { $set: { status: 'REJECTED', comments, reviewedAt: now, reviewedBy: reviewer?.name ?? currentUser.email, updatedAt: now } }
  );
