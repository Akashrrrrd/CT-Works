          ...
          {data?.workspaces.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {([...(data?.workspaces || [])]).sort((a, b) => {
                const nameA = sanitizeWorkspaceName(a.name);
                const nameB = sanitizeWorkspaceName(b.name);
                const yearA = nameA.match(/\b(20\d{2})\b/) ? parseInt(nameA.match(/\b(20\d{2})\b/)![1]) : new Date(a.createdAt).getFullYear();
                const yearB = nameB.match(/\b(20\d{2})\b/) ? parseInt(nameB.match(/\b(20\d{2})\b/)![1]) : new Date(b.createdAt).getFullYear();
                return yearB - yearA;
              }).map(ws => {
                const displayName = sanitizeWorkspaceName(ws.name);
                const displayDescription = sanitizeWorkspaceDescription(ws.description);
                const year = displayName.match(/\b(20\d{2})\b/)?.[1] || new Date(ws.createdAt).getFullYear();
                return (
                  <Link key={ws.id} href={`/workspaces/${ws.id}`}>
                    <Card className="cursor-pointer hover:border-primary/50 transition-colors h-full flex flex-col justify-between">
                      <CardHeader>
