...
export default function DashboardPage() {
 ...
 {([...(data?.workspaces || [])]).sort((a, b) => {
                const yearA = a.name.match(/\b(20\d{2})\b/) ? parseInt(a.name.match(/\b(20\d{2})\b/)![1]) : new Date(a.createdAt).getFullYear();
                const yearB = b.name.match(/\b(20\d{2})\b/) ? parseInt(b.name.match(/\b(20\d{2})\b/)![1]) : new Date(b.createdAt).getFullYear();
                return yearB - yearA;
              }).map(ws => {
                const year = ws.name.match(/\b(20\d{2})\b/)?.[1] || new Date(ws.createdAt).getFullYear();
                return (
