...
export default function ProjectDetailPage() {
  ...
  const load = () => {
    ...
    fetch(`/api/workspaces/${workspaceId}/hierarchy`)
      .then(r => r.json())
      .then(d => {
        const project = (d.tree ?? []).find((s: any) => s.id === projectId);
        if (project) { setProjectName(project.name); setBays(project.bays ?? []); }
      })
  ...
  const addBay = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/workspaces/${workspaceId}/projects/${projectId}/bays`, {
  ...
  const addIED = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true); setError('');
    try {
      const res = await fetch(`/api/workspaces/${workspaceId}/projects/${projectId}/bays/${iedBayId}/ieds`, {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href={`/workspaces/${workspaceId}/projects`}>
            <Button variant="ghost" size="sm" className="gap-1"><ArrowLeft className="h-4 w-4" />Projects</Button>
          </Link>
